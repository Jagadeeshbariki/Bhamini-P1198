...
const BYPPage: React.FC = () => {
    const [data, setData] = useState<PoultryRecord[]>([]);
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const parsedData: PoultryRecord[] = rawData.map(row => {
                const hhId = getFuzzyValue(row, ['HH_id', 'HH ID', 'HHID', 'Farmer ID', 'FARMERID']) || '';
                const normHhId = normalizeId(hhId);
                const activity = getFuzzyValue(row, ['Activity']) || 'Poultry';
                ...
                for (const cRow of contribRows) {
                    ...
                    // Robust matching for Poultry / BYP
                    const isPoultryActivity = activity.toUpperCase().includes('POULTRY') || activity.toUpperCase().includes('BYP');
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <p className="text-sm font-medium text-gray-500">Loading Poultry Data...</p>
        </div>
    );
    ...
    return (
        <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">BYP Poultry Dashboard</h1>
