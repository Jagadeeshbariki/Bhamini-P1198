...
const oldRegex = /const indTarget = farmerTargetMap\.get\(\`\\\${normId}-\\\${normalizedActivity}\`\) \|\| 0;/g;

const replacementFunc = `let indTarget = farmerTargetMap.get(\`\${normId}-\${normalizedActivity}\`) || 0;
                      if (indTarget === 0) {
...
content = content.replace(oldRegex, replacementFunc);

fs.writeFileSync('components/ContributionPage.tsx', content);
console.log('Fixed ind targets old format');
